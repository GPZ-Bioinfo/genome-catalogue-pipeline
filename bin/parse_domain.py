...
def load_clusters(clusters_file):
    ...
    with open(clusters_file, "r") as file_in:
        next(file_in)
        for line in file_in:
            #*cluster_rep = line.strip().split("\t")[-1].split(",")[0].rsplit('.', 1)[0] use split *get the rep w/o extension MGYG000000004.fna to MGYG000000004
            cluster_rep = line.strip().split("\t")[1]
